...
public sealed class NetworkPlayer(IConnection connection, bool isHost) : IMessageSender, IVisibilityTracker, IObjectOwner, ISceneLoader
{
    private static readonly ILogger logger = LogFactory.GetLogger(typeof(NetworkPlayer));

    private readonly HashSet<NetworkIdentity> _visList = [];
    ...
    public void SetAuthentication(PlayerAuthentication authentication, bool allowReplace = false)
    {
    ...
    private readonly HashSet<NetworkIdentity> _ownedObjects = [];
