...
public class MessageHandler(IObjectLocator objectLocator, bool disconnectOnException, bool rethrowException = false) : IMessageReceiver
{
    private static readonly ILogger logger = LogFactory.GetLogger<MessageHandler>();

    private readonly bool _disconnectOnException = disconnectOnException;
    private readonly bool _rethrowException = rethrowException;
    private readonly IObjectLocator _objectLocator = objectLocator;

    internal readonly Dictionary<int, Handler> _messageHandlers = [];
    ...
    internal class Handler(NetworkMessageDelegate @delegate, bool allowUnauthenticated)
    {
        public readonly NetworkMessageDelegate Delegate = @delegate;
        public readonly bool AllowUnauthenticated = allowUnauthenticated;
    }
}
