...
public partial class SessionIdAuthenticator : NetworkAuthenticator<SessionKeyMessage>
{
    ...
    /// </summary>
    private readonly Dictionary<SessionKey, SessionData> _sessions = [];
    /// <summary>
    /// Key for player
    /// </summary>
    private readonly Dictionary<NetworkPlayer, SessionKey> _playerKeys = [];
    ...
    internal readonly struct SessionKey(ArraySegment<byte> bytes) : IEquatable<SessionKey>
    {
        public ArraySegment<byte> Buffer { get; } = bytes;
        private readonly int _hash = CalculateHash(bytes);

        public SessionKey(byte[] array) : this(new ArraySegment<byte>(array)) { }

        public readonly bool Equals(SessionKey other)
        {
        ...
        {
            return obj is SessionKey key && Equals(key);
        }
