...
public struct NetworkBehaviorSyncvar
{
    ...
    internal readonly uint NetId => _component != null ? _component.Identity.NetId : _netId;
    internal readonly int ComponentId => _component != null ? _component.ComponentIndex : _componentId;
    ...
    public INetworkNode Value
    {
        get
        {
            ...
            return _objectLocator is IObjectLocator locator && locator.TryGetIdentity(NetId, out var result)
                ? result.NetworkBehaviours[_componentId]
                : null;
        }
