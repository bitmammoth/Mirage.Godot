...
namespace Mirage.Godot.Scripts.Syncing.Collections;

public class SyncSet<T>(ISet<T> objects) : ISet<T>, ISyncObject
{
    protected readonly ISet<T> _objects = objects;

    public int Count => _objects.Count;
    public bool IsReadOnly { get; private set; }
    void ISyncObject.SetShouldSyncFrom(bool shouldSync) => IsReadOnly = !shouldSync;
    void ISyncObject.SetNetworkBehaviour(INetworkNode networkBehaviour) { }
    ...
    public bool Contains(T item) => _objects.Contains(item);
    ...
    {
        var elements = new List<T>(_objects);
    ...
    public bool IsProperSubsetOf(IEnumerable<T> other) => _objects.IsProperSubsetOf(other);

    public bool IsProperSupersetOf(IEnumerable<T> other) => _objects.IsProperSupersetOf(other);

    public bool IsSubsetOf(IEnumerable<T> other) => _objects.IsSubsetOf(other);

    public bool IsSupersetOf(IEnumerable<T> other) => _objects.IsSupersetOf(other);

    public bool Overlaps(IEnumerable<T> other) => _objects.Overlaps(other);

    public bool SetEquals(IEnumerable<T> other) => _objects.SetEquals(other);
...
public class SyncHashSet<T>(IEqualityComparer<T> comparer) : SyncSet<T>(new HashSet<T>(comparer ?? EqualityComparer<T>.Default))
{
    public SyncHashSet() : this(EqualityComparer<T>.Default) { }

    // allocation free enumerator

    public new HashSet<T>.Enumerator GetEnumerator() => ((HashSet<T>)_objects).GetEnumerator();
}

public class SyncSortedSet<T>(IComparer<T> comparer) : SyncSet<T>(new SortedSet<T>(comparer ?? Comparer<T>.Default))
{
    public SyncSortedSet() : this(Comparer<T>.Default) { }

    // allocation free enumerator

    public new SortedSet<T>.Enumerator GetEnumerator() => ((SortedSet<T>)_objects).GetEnumerator();
}
