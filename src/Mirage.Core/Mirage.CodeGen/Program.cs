...
internal class Program
{
    private static void Main(string[] argsArray)
    {
        try
        {
            ...
            var compiledAssembly = new CompiledAssembly(dllPath, references, []);
            var weaverLogger = new WeaverLogger(false);
            var weaver = new Weaver.Weaver(weaverLogger);
            var result = weaver.Process(compiledAssembly, [.. hints]);
...
{
    public readonly byte[] PeData = peData;
    public readonly byte[] PdbData = pdbData;
}
