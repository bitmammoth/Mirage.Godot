...
public class PropertySiteProcessor(IWeaverLogger logger)
{
    ...
    private Instruction ProcessInstruction(MethodDefinition md, Instruction instr, SequencePoint sequencePoint)
    {
        ...
        {
            FieldReference resolved = opFieldst.Resolve() ?? opFieldst.DeclaringType.Resolve().GetField(opFieldst.Name);
