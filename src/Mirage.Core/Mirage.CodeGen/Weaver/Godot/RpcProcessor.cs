...
public abstract class RpcProcessor(ModuleDefinition module, Readers readers, Writers writers, IWeaverLogger logger)
{
    protected readonly ModuleDefinition _module = module;
    protected readonly Readers _readers = readers;
    protected readonly Writers _writers = writers;
    protected readonly IWeaverLogger _logger = logger;
    ...
    protected ValueSerializer[] GetValueSerializers(MethodDefinition method)
    {
        ...
        for (var i = 0; i < method.Parameters.Count; i++)
        {
            ...
            try
            {
                serializers[i] = ValueSerializerFinder.GetSerializer(method, method.Parameters[i], _writers, _readers);
            }
            catch (SerializeFunctionException e)
            {
                _logger.Error(e, method.DebugInformation.SequencePoints.FirstOrDefault());
                error = true;
            }
            catch (ValueSerializerException e)
            {
                _logger.Error(e.Message, method);
                error = true;
            }
        }
    ...
    private void WriteArgument(ILProcessor worker, VariableDefinition writer, ParameterDefinition param, ValueSerializer serializer)
    {
        ...
        serializer.AppendWriteParameter(_module, worker, writer, param);
    }
    ...
    private void ReadArgument(ILProcessor worker, ParameterDefinition readerParameter, ParameterDefinition senderParameter, ParameterDefinition param, ValueSerializer serializer)
    {
        ...
        serializer.AppendRead(_module, worker, readerParameter, param.ParameterType);
    }
    ...
    protected static void ValidateMethod(MethodDefinition method)
    {
    ...
    /// </summary>
    /// <exception cref="RpcException">Throws when parameter are invalid</exception>
    protected static ReturnType ValidateReturnType(MethodDefinition md, RemoteCallType callType, RpcTarget rpcTarget)
    {
        // void is allowed
        ...
        // Task is allowed
        var TaskType = typeof(Task<int>).GetGenericTypeDefinition();
        if (returnType.Is(TaskType))
            return ReturnType.Task;

        throw new RpcException($"Use Task<{md.ReturnType}> to return values from [ClientRpc] or [ServerRpc]", md);
    }
    ...
    /// </summary>
    /// <exception cref="RpcException">Throws when parameter are invalid</exception>
    private static void ValidateParameter(MethodReference method, ParameterDefinition param, RemoteCallType callType, bool firstParam)
    {
        if (param.IsOut)
    ...
    private void FixRemoteCallToBaseMethod(TypeDefinition type, MethodDefinition method, MethodDefinition generatedMethod)
    {
        ...
        foreach (var instruction in generatedMethod.Body.Instructions)
        {
            ...
            var userCodeReplacement = type.GetMethodInBaseType(targetName) ?? throw new RpcException($"Could not find base method for {userCodeName}", generatedMethod);
