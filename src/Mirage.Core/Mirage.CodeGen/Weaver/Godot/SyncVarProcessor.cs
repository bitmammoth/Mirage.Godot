...
/// </summary>
public class SyncVarProcessor(ModuleDefinition module, Readers readers, Writers writers, PropertySiteProcessor propertySiteProcessor)
{
    ...
    private readonly PropertySiteProcessor _propertySiteProcessor = propertySiteProcessor;

    private FoundNetworkBehaviour _behaviour;

    public void ProcessSyncVars(TypeDefinition td, IWeaverLogger logger)
    {
        _behaviour = new FoundNetworkBehaviour(_module, td);
        ...
        foreach (var fd in td.Fields.ToArray())
        {
            ...
            try
            {
                ...
                {
                    var syncVar = _behaviour.AddSyncVar(fd);
        ...
        _behaviour.SetSyncVarCount();

        GenerateSerialization();
        GenerateDeserialization();
    }

    private static bool IsValidSyncVar(FieldDefinition field)
    {
    ...
    private void ProcessSyncVar(FoundSyncVar syncVar)
    {
        ...
        syncVar.SetWrapType();
        syncVar.ProcessAttributes(_writers, _readers);
        ...
        var propertyDefinition = new PropertyDefinition("Network" + originalName, PropertyAttributes.None, syncVar.OriginalType)
        {
            GetMethod = get,
            SetMethod = set,
            DeclaringType = fd.DeclaringType
        };
        //add the methods and property to the type.
        fd.DeclaringType.Properties.Add(propertyDefinition);
        _propertySiteProcessor.Setters[fd] = set;

        if (syncVar.IsWrapped)
            _propertySiteProcessor.Getters[fd] = get;
    }
    ...
    private MethodDefinition GenerateSyncVarSetter(FoundSyncVar syncVar)
    {
        ...
        var syncVarEqual = _module.ImportReference(() => NetworkNodeExtensions.SyncVarEqual<object>(default, default));
        ...
        if (syncVar.HasHook)
        {
            ...
            {
                worker.Append(worker.Create(OpCodes.Ldarg_0));
                worker.Append(worker.Create(OpCodes.Call, (Obj.NetworkBehaviour nb) => nb.HasAuthority()));
            ...
            if (syncVar.InvokeHookOnServer)
                ...
                worker.Append(worker.Create(OpCodes.Call, (Obj.NetworkBehaviour nb) => nb.IsServer()));
            else
                worker.Append(worker.Create(OpCodes.Call, (Obj.NetworkBehaviour nb) => nb.IsMainCharacter()));
    ...
    private void WriteLoadField(ILProcessor worker, FoundSyncVar syncVar)
    {
        ...
        if (syncVar.IsWrapped)
        {
            ...
            var getter = _module.ImportReference(fd.FieldType.Resolve().GetMethod("get_Value"));
    ...
    private void WriteStoreField(ILProcessor worker, ParameterDefinition valueParam, FoundSyncVar syncVar)
    {
        ...
        if (syncVar.IsWrapped)
        {
            ...
            var setter = _module.ImportReference(fd.FieldType.Resolve().GetMethod("set_Value"));
    ...
    private void WriteCallHookMethod(ILProcessor worker, MethodDefinition hookMethod, int argCount, VariableDefinition oldValue, FoundSyncVar syncVarField)
    {
        ...
        void WriteEndFunctionCall()
        {
            ...
            worker.Append(worker.Create(OpCall, _module.ImportReference(hookMethodReference)));
        }
    }
    ...
    private void WriteCallHookEvent(ILProcessor worker, EventDefinition @event, int argCount, VariableDefinition oldValue, FoundSyncVar syncVarField)
    {
        ...
        var invoke = _module.ImportReference(actionType.GetMethod("Invoke"));
    ...
    {
        Weaver.DebugLog(_behaviour.TypeDefinition, "GenerateSerialization");

        // dont create if there are no syncvars
        if (_behaviour.SyncVars.Count == 0)
            return;

        var helper = new SerializeHelper(_module, _behaviour);
        ...
        {
            foreach (var syncVar in _behaviour.SyncVars)
            {
    ...
    private void WriteFromField(ILProcessor worker, ParameterDefinition writerParameter, FoundSyncVar syncVar)
    {
        ...
        syncVar.ValueSerializer.AppendWriteField(_module, worker, writerParameter, null, fieldRef);
    }

    private void GenerateDeserialization()
    {
        Weaver.DebugLog(_behaviour.TypeDefinition, "GenerateDeSerialization");

        // dont create if there are no syncvars
        if (_behaviour.SyncVars.Count == 0)
            return;

        var helper = new DeserializeHelper(_module, _behaviour);
        ...
        helper.WriteIfInitial(() =>
        {
            ...
            var oldValues = new VariableDefinition[_behaviour.SyncVars.Count];
            for (var i = 0; i < _behaviour.SyncVars.Count; i++)
            {
                var syncVar = _behaviour.SyncVars[i];
    ...
    private void ReadToField(ILProcessor worker, ParameterDefinition readerParameter, FoundSyncVar syncVar)
    {
        ...
        syncVar.ValueSerializer.AppendRead(_module, worker, readerParameter, syncVar.FieldDefinition.FieldType);
